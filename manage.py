...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Manage the local Movies Recommender Docker installation."
    )
    ...
    def common(p):
        p.add_argument(
            "--dev",
            action="store_true",
            help="Use compose.dev.yaml and local source builds.",
        )
    ...
    common(dataset)
    dataset.add_argument("--source", choices=("existing", "download", "zip"))
    dataset.add_argument("--zip-path", type=Path)
    dataset.add_argument(
        "--preset", choices=("recommended", "defaults", "custom"), default="recommended"
    )
    dataset.add_argument(
        "--cleanup", choices=("none", "standard", "minimal"), default="none"
    )
    dataset.add_argument("--skip-posters", action="store_true")
    dataset.add_argument("--audit", action="store_true")
    dataset.add_argument("--yes", action="store_true")
    for name in (
        "candidate-limit",
        "candidate-min-ratings",
        "candidate-min-year",
        "candidate-max-year",
        "candidate-min-tags",
        "max-tags-per-movie",
        ...
        dataset.add_argument("--" + name, type=int)
    for name, help_text in (
        ("install", "Build recommender models and start the API"),
        ("rebuild-models", "Rebuild selected recommender models"),
        ("backend-install", "Build recommender models and start the API"),
    ):
        build = subs.add_parser(name, help=help_text)
        common(build)
        build.add_argument("--algorithms", default="all")
        build.add_argument("--item-knn-variant")
        build.add_argument("--bmf-variant")
        clean = build.add_mutually_exclusive_group()
        clean.add_argument("--clean", action="store_true")
        clean.add_argument("--no-clean", action="store_true")
        build.add_argument("--audit", action="store_true")
        build.add_argument("--yes", action="store_true")
    for name, help_text in (
        ("start", "Start the API using existing recommender artifacts"),
        ("deploy", "Alias for start"),
        ("backend-start", "Start only the API using existing artifacts"),
    ):
        start = subs.add_parser(name, help=help_text)
        common(start)
    start_all = subs.add_parser(
        "start-all", help="Explicitly start the API and optional frontend"
    )
    common(start_all)
    audit = subs.add_parser(
        "audit-models", help="Audit existing recommender artifacts without rebuilding"
    )
    common(audit)
    for name in (
        "restart",
    ...
    dev_rebuild = subs.add_parser(
        "dev-rebuild", help="Rebuild a development service after dependency changes"
    )
    dev_rebuild.add_argument("service", choices=("frontend", "backend", "all"))
    subs.add_parser("backend", help=argparse.SUPPRESS)
    subs.add_parser("frontend", help=argparse.SUPPRESS)
    return root
...
def run(args: list[str], *, env: dict[str, str] | None = None) -> int:
    try:
        return subprocess.run(args, cwd=ROOT, env=env, shell=False).returncode
...
def update_env(updates: dict[str, str]) -> None:
    ...
    original = (
        ...
        if ENV_FILE.exists()
        else EXAMPLE_ENV.read_text(encoding="utf-8").splitlines()
    )
...
def validate_dataset(data_dir: Path) -> tuple[bool, str]:
    root = data_dir / "offline_dataset"
    required = (
        root / "manifest.json",
        root / "csv" / "public_movies.csv",
        root / "csv" / "collaborative_support_movies.csv",
        root / "csv" / "collaborative_ratings.csv",
    )
    missing = [str(p) for p in required if not p.is_file() or p.stat().st_size == 0]
    posters = root / "images" / "posters"
    if not posters.is_dir():
        missing.append(str(posters))
    if missing:
        return False, "Missing required dataset paths: " + ", ".join(missing)
    return (
        True,
...
def configured_data_dir(args) -> Path:
    configured = Path(
        args.data_dir or read_env().get("DATA_DIR", "./data")
    ).expanduser()
...
def dev(args) -> int:
    ...
    if run(compose_args(True) + ["up", "-d", "api", "frontend"]):
        print(
            "Could not start development services. If a configured port is in use, stop the conflicting published service first.",
            file=sys.stderr,
        )
        return 1
    ...
    if not ready:
        print(
            "The development frontend remains running; inspect `python manage.py dev-logs api`.",
            file=sys.stderr,
        )
        return 1
...
def dev_stop(args) -> int:
    ...
    if code == 0:
        print(
            "Development API and frontend stopped. Persistent data and dependency volumes were preserved."
        )
    return code
...
def dev_status(args) -> int:
    ...
    print(
        "Frontend URL: "
        + _configured_url("FRONTEND_BIND_HOST", "FRONTEND_PORT", "5173")
    )
    print(
        "Backend URL: " + _configured_url("BACKEND_BIND_HOST", "BACKEND_PORT", "18014")
    )
    return 0
...
def dev_rebuild(args) -> int:
    services = (
        ("api", "frontend")
        if args.service == "all"
        else (("api",) if args.service == "backend" else ("frontend",))
    )
    if not ensure_docker(True):
        return 1
...
def dataset(args) -> int:
    ...
    updates = {"DATA_DIR": configured_data_env_value(args, data)}
    if args.source == "zip" and (not args.zip_path or not args.zip_path.is_file()):
        print("--source zip requires an existing regular --zip-path.", file=sys.stderr)
        return 1
    if args.source != "zip" and args.zip_path:
        print("--zip-path is only valid with --source zip.", file=sys.stderr)
        return 1
    if args.non_interactive and not args.source:
        print("--non-interactive dataset requires --source.", file=sys.stderr)
        return 1
    if args.non_interactive and not args.yes:
        print("--non-interactive dataset requires --yes.", file=sys.stderr)
        return 1
    if not args.non_interactive and not read_env().get(
        "MOVIES_RECOMMENDER_TMDB_BEARER_TOKEN"
    ):
        token = getpass.getpass(
            "TMDB bearer token (leave blank to use existing/no enrichment): "
        ).strip()
        if token:
            updates["MOVIES_RECOMMENDER_TMDB_BEARER_TOKEN"] = token
    update_env(updates)
    if not ensure_docker(args.dev):
        return 1
    ...
    if args.non_interactive:
        ...
        command += ["--cleanup", args.cleanup]
    if args.source == "zip":
        command += ["--zip-path", "/input/ml-32m.zip"]
    if args.skip_posters:
        command.append("--skip-posters")
    if args.audit:
    ...
    for name in (
        ...
        "max_tags_per_movie",
        "public_limit",
        "collaborative_core_limit",
        "catalog_min_ratings",
        "public_min_year",
        "collaborative_min_year",
    ):
    ...
    if args.yes:
        command.append("--yes")
    code = run(command)
    print(f"Dataset location: {data / 'offline_dataset'}") if code == 0 else None
    return code


def profiles(args) -> dict:
    command = compose_args(args.dev) + [
        "--profile",
        "maintenance",
        "run",
        "--rm",
    ...
    try:
        result = subprocess.run(
            command, cwd=ROOT, capture_output=True, text=True, shell=False
        )
    except OSError as exc:
        raise RuntimeError(
            "Docker Compose is unavailable for profile discovery."
        ) from exc
    if result.returncode or not result.stdout.strip():
        raise RuntimeError("Could not read recommender profile catalogue.")
    try:
        payload = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise RuntimeError(
            "Recommender profile catalogue returned invalid JSON."
        ) from exc
    if not isinstance(payload.get("itemKnn"), list) or not isinstance(
        payload.get("biasedMatrixFactorization"), list
    ):
        raise RuntimeError(
            "Recommender profile catalogue is missing expected profile groups."
        )
    return payload
...
def rebuild_models(args) -> int:
    ...
    if not ensure_docker(args.dev):
        return 1
    try:
        catalogue = profiles(args)
    except RuntimeError as exc:
        print(str(exc), file=sys.stderr)
        return 1
    item = args.item_knn_variant or next(
        (p["variantId"] for p in catalogue["itemKnn"] if p.get("recommended")), None
    )
    bmf = args.bmf_variant or (
        catalogue["biasedMatrixFactorization"][0].get("variantId")
        if catalogue["biasedMatrixFactorization"]
        else None
    )
    ...
    if not args.non_interactive:
        ...
        bmf = _choose_profile(
            "Select BMF variant", catalogue["biasedMatrixFactorization"], bmf
        )
        raw_algorithms = input("Algorithms [all]: ").strip() or "all"
        args.algorithms = raw_algorithms
    requested = (
        ALGORITHMS if args.algorithms == "all" else tuple(args.algorithms.split(","))
    )
    if not requested or any(name not in ALGORITHMS for name in requested):
        print(
            "Algorithms must be a non-empty comma-separated selection of supported names.",
            file=sys.stderr,
        )
        return 1
    if item not in {p.get("variantId") for p in catalogue["itemKnn"]} or bmf not in {
        p.get("variantId") for p in catalogue["biasedMatrixFactorization"]
    }:
        print("Unsupported recommender variant.", file=sys.stderr)
        return 1
    selected = requested
    if args.non_interactive and not args.yes:
        print("--non-interactive deploy requires --yes.", file=sys.stderr)
        return 1
    _print_deploy_summary(data, selected, item, bmf, clean_enabled, False, args.dev)
    if not args.non_interactive and not _ask_yes_no(
        "Build selected recommender models?", True
    ):
        return 0
    update_env(
        {
            "DATA_DIR": configured_data_env_value(args, data),
            "MOVIES_RECOMMENDER_ACTIVE_COLLABORATIVE_MODEL_VARIANT": item,
            "MOVIES_RECOMMENDER_BIASED_MATRIX_FACTORIZATION_MODEL_VARIANT": bmf,
        }
    )
    command = compose_args(args.dev) + [
        "--profile",
    ...
    if args.audit and audit_models(args):
        return 1
    if run(compose_args(args.dev) + ["up", "-d", "--force-recreate", "api"]):
        return 1
    if not wait_ready(read_env().get("BACKEND_PORT", "8014")):
        return 1
    return 0
...
def install(args) -> int:
    ...
    if compatible:
        if args.non_interactive or _ask_yes_no(
            "Compatible recommender models exist. Reuse them?", True
        ):
            return start_backend(args)
        if not _ask_yes_no("Rebuild recommender models?", False):
            return 0
...
def start_backend(args) -> int:
    ...
    if not compatible:
        print(
            "Cannot start backend with existing recommender artifacts: " + reason,
            file=sys.stderr,
        )
        print(
            "Run `python manage.py rebuild-models` to construct compatible artifacts.",
            file=sys.stderr,
        )
        return 1
    if not ensure_docker(args.dev):
...
def frontend_install(args) -> int:
    if not ensure_docker(args.dev):
        return 1
    if run(compose_args(args.dev) + ["--profile", "frontend", "pull", "frontend"]):
        return 1
    return frontend_start(args, pull=False)
...
def frontend_start(args, *, pull: bool = False) -> int:
    if not ensure_docker(args.dev):
        return 1
    if pull and run(
        compose_args(args.dev) + ["--profile", "frontend", "pull", "frontend"]
    ):
        return 1
    return run(
        compose_args(args.dev) + ["--profile", "frontend", "up", "-d", "frontend"]
    )


def frontend_restart(args) -> int:
    return run(
        compose_args(args.dev) + ["--profile", "frontend", "restart", "frontend"]
    )
...
def backend_restart(args) -> int:
    if run(compose_args(args.dev) + ["restart", "api"]):
        return 1
...
def service_is_installed(args, service: str) -> bool:
    command = (
        compose_args(args.dev)
        + (["--profile", "frontend"] if service == "frontend" else [])
        + ["ps", "-q", "--all", service]
    )
    try:
        result = subprocess.run(
...
def restart_installed_services(args) -> int:
    api = service_is_installed(args, "api")
    frontend = service_is_installed(args, "frontend")
    if not api and not frontend:
        print(
            "No installed services were found. Start the backend or frontend explicitly."
        )
        return 0
    if api and backend_restart(args):
        return 1
    if frontend and frontend_restart(args):
        return 1
    return 0


def stop_installed_services(args) -> int:
    if not service_is_installed(args, "api") and not service_is_installed(
        args, "frontend"
    ):
        print("No installed services were found.")
        return 0
    code = run(
        compose_args(args.dev) + ["--profile", "frontend", "down", "--remove-orphans"]
    )
    if code == 0:
        print("Persistent DATA_DIR contents were not deleted.")
    return code
...
def frontend_status(args) -> int:
    ...
    print(
        "Frontend URL: "
        + _configured_url("FRONTEND_BIND_HOST", "FRONTEND_PORT", "15173")
    )
...
def audit_models(args) -> int:
    ...
    if not compatible:
        print(
            "Cannot audit incompatible recommender artifacts: " + reason,
            file=sys.stderr,
        )
        return 1
    if not ensure_docker(args.dev):
...
def validate_active_models(data: Path) -> tuple[bool, str]:
    ...
    if not state.is_file():
        return False, "model compatibility state is missing"
    try:
        payload = json.loads(state.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return False, "model compatibility state is unreadable"
    if payload.get("datasetFingerprint") != _dataset_fingerprint(data):
        return False, "model artifacts are stale for the current offline dataset"
    variants = read_env()
    required = (
    ...
    return (
        (False, "required active artifacts are missing: " + ", ".join(missing))
        if missing
        else (True, "compatible")
    )
...
def _write_model_dataset_state(data: Path) -> None:
    ...
    state.write_text(
        json.dumps(
            {
                "datasetFingerprint": _dataset_fingerprint(data),
...
def wait_ready(port: str) -> bool:
    ...
    for _ in range(30):
        ...
        time.sleep(1)
    print(
        f"Backend is not ready: {latest}. Run python manage.py status.", file=sys.stderr
    )
    return False
...
def dispatch(args) -> int:
    ...
    if getattr(args, "dev", False):
        print(
            "--dev is deprecated; use `python manage.py dev` for normal local development.",
            file=sys.stderr,
        )
    if args.command == "dev":
        return dev(args)
    ...
    if args.command == "dev-logs":
        return dev_logs(args)
    if args.command == "dev-rebuild":
        return dev_rebuild(args)
    if args.command == "backend":
        return backend_management()
    if args.command == "frontend":
        return frontend_management()
    if args.command == "dataset":
        return dataset(args)
    if args.command in {"install", "backend-install"}:
        return install(args)
    if args.command == "rebuild-models":
        return rebuild_models(args)
    if args.command in {"start", "deploy", "backend-start"}:
        return start_backend(args)
    if args.command == "start-all":
        return start_all(args)
    if args.command == "audit-models":
        return audit_models(args)
    if args.command in {"restart"}:
        return restart_installed_services(args)
    if args.command == "backend-restart":
        return backend_restart(args)
    if args.command == "backend-stop":
        return backend_stop(args)
    if args.command == "frontend-install":
        return frontend_install(args)
    if args.command == "frontend-start":
        return frontend_start(args)
    if args.command == "frontend-restart":
        return frontend_restart(args)
    if args.command == "frontend-stop":
        return frontend_stop(args)
    if args.command == "frontend-status":
        return frontend_status(args)
    if args.command == "status":
        run(compose_args(args.dev) + ["ps"])
        data = configured_data_dir(args)
        dataset_ok, _ = validate_dataset(data)
        models_ok, _ = (
            validate_active_models(data) if dataset_ok else (False, "dataset missing")
        )
        ...
        print(
            ...
            + (
                "running"
                if service_is_installed(args, "api")
                and wait_ready(read_env().get("BACKEND_PORT", "18014"))
                else "stopped / unhealthy"
            )
        )
        print(
            "API URL: " + _configured_url("BACKEND_BIND_HOST", "BACKEND_PORT", "18014")
        )
        print(
            "Frontend:  "
            + ("running" if service_is_installed(args, "frontend") else "not installed")
        )
        print(
            "Frontend URL: "
            + _configured_url("FRONTEND_BIND_HOST", "FRONTEND_PORT", "15173")
    ...
    if args.command == "stop":
        return stop_installed_services(args)
    return 1
...
def backend_management() -> int:
    ...
    return (
        0
        if choice == "0"
        else dispatch(parser().parse_args([command])) if command else 1
    )
...
def _ask_yes_no(question: str, default: bool) -> bool:
    try:
        value = input(f"{question} [{'Y/n' if default else 'y/N'}] ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        return False
...
def _resolve_clean(args, *, interactive: bool) -> bool:
    ...
    if args.no_clean:
        return False
    return (
        _ask_yes_no("Remove optional recommender exports?", True)
        if interactive
        else True
    )
...
def _choose_profile(title: str, values: list[dict], default: str | None) -> str | None:
    ...
    for index, value in enumerate(values, start=1):
        print(
            f"{index}. {value.get('label', value.get('variantId'))}"
            + (" [recommended]" if value.get("recommended") else "")
        )
    try:
        ...
        return default
    return (
        values[int(choice) - 1].get("variantId")
