...
function MovieRatingControl({ movie, rating, onRate, className = '' }) {
  const [hoveredRating, setHoveredRating] = useState(null)
  const previewRating = hoveredRating || rating || 0
  const title = movie.displayTitle || movie.title || 'película'

  return (
    <div className={`poster-stars ${className}`.trim()} aria-label={`Valorar ${title}`} onMouseLeave={() => setHoveredRating(null)}>
      {[1, 2, 3, 4, 5].map((value) => (
        <button
          key={value}
          type="button"
