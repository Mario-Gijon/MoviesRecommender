...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "python -m pipelines.dataset_generation.run_movielens_32m_pipeline \\\n"
...
def _build_stage_command(stage: str, args: argparse.Namespace) -> list[str]:
    ...
    if stage == "candidates":
        command.extend(
            [
                "-m",
                "pipelines.dataset_generation.build_movielens_32m_candidates",
                "--limit",
    ...
    if stage == "enrich":
        command.extend(["-m", "pipelines.dataset_generation.enrich_movielens_32m_with_tmdb"])
        if args.resume_tmdb:
    ...
    if stage == "ratings":
        command.extend(["-m", "pipelines.dataset_generation.build_demo_ratings_from_movielens_32m"])
        return command

    if stage == "export":
        command.extend(["-m", "pipelines.dataset_generation.export_offline_dataset_from_movielens_32m"])
        return command

    if stage == "posters":
        command.extend(["-m", "pipelines.dataset_generation.download_offline_movie_posters"])
        return command

    if stage == "audit":
        command.extend(["-m", "pipelines.dataset_generation.audit_offline_dataset"])
        return command
