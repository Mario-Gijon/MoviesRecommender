...
EXPLANATION_TEMPLATES_PATH = Path(__file__).resolve().parent / "explanation_templates.json"
PLACEHOLDER_RE = re.compile(r"\{([a-zA-Z_][a-zA-Z0-9_]*)\}")
REQUIRED_GROUPS = {
    "headline",
    "positive_connection_reason",
    "collaborative_support_reason",
...
def load_explanation_templates() -> ExplanationTemplateBank:
    if not EXPLANATION_TEMPLATES_PATH.exists():
        raise RuntimeError(f"Collaborative explanation template file is missing: {EXPLANATION_TEMPLATES_PATH}")
...
def _validate_template_bank(raw_bank: dict) -> None:
    for key in ("version", "description", "selection", "groups"):
        if key not in raw_bank:
            raise RuntimeError(f"Collaborative explanation template JSON is missing top-level key: {key}")

    groups = raw_bank["groups"]
    missing_groups = REQUIRED_GROUPS - set(groups.keys())
    if missing_groups:
        missing_text = ", ".join(sorted(missing_groups))
        raise RuntimeError(f"Collaborative explanation template JSON is missing required groups: {missing_text}")

    for group_name, styles in groups.items():
        if not isinstance(styles, dict):
            raise RuntimeError(f"Collaborative template group {group_name} must be an object keyed by style.")
        for style_name, templates in styles.items():
            if not isinstance(templates, list):
                raise RuntimeError(f"Collaborative template group {group_name}.{style_name} must be a list.")
