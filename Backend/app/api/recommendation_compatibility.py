...
def recommend_content_based_compatibility(
...
def recommend_collaborative_compatibility(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    "The selected recommendation model is currently unavailable."
                ),
            },
        ) from exc
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    "The recommendation input is invalid for the selected "
                    "algorithm."
                ),
            },
        ) from exc
...
def _build_collaborative_profile(
    *,
    ...
    non_neutral_rating_count = sum(
        1 for item in request.ratings if item.rating != 3
    )
    positive_rating_count = sum(
        1 for item in request.ratings if item.rating >= 4
    )
    negative_rating_count = sum(
        1 for item in request.ratings if item.rating <= 2
    )
